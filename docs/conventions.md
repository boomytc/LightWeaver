...
- 音色套：一套引用 `library:voice.prompt`。参考是一支克隆源，或一段 instruct。instruct 铸出来的是试听，不要标成克隆源。出片固定 Hi-Fi clone（试听优先，`ref_audio` + 逐字稿）。人在 Studio `/voices` 铸、听、留；`tts` 不改库，不加 `--seed`。要出的语言用 `weaver langs set --project <id> --langs zh`。VoxCPM2 控制台：https://platform.modelbest.cn/console/login?ref=B08B4DDF
