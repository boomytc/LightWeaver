...
- 音色套：一套引用 `library:voice.prompt`。克隆源二选一：上传录音，或 instruct 铸完再收。铸出的 wav 就是克隆源。出片固定 Hi-Fi clone（`ref_audio` + 逐字稿）。人在 Studio `/voices` 铸、听、留；`tts` 不改库，不加 `--seed`。要出的语言用 `weaver langs set --project <id> --langs zh`。VoxCPM2 控制台：https://platform.modelbest.cn/console/login?ref=B08B4DDF
