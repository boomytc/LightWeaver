...
| 共享音色 | voice | `library/voices/prompt-{zh,en}.wav` + `.txt`，一条资产 `files`/`texts`/`styles` | 人 / assets skill | `runTts`（VoxCPM2 公开克隆：`ref_audio`；风格写进 input 前缀） | 提交 | `library:voice.prompt`（各语言同一引用；`film.langs` 决定出哪种） |
