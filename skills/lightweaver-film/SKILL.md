...
4. 真 lab 静帧。不要手绘假 UI。
5. 双语成对写完再 TTS。
6. 先形状后媒体；能复用就不重生。`validate` error 未清不得交付；`!isRenderable` 不得 `render`。人在 Studio `/voices` `/library` 监管库，在 `/f/<id>` 点名 `voices` / `kit`。agent 只按点名用，不在站里排场或出片，不用 `tts --seed` 改库里的参考声。
